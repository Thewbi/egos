# Links
- https://github.com/yhzhang0128/egos-2000
- https://egos.fun/book/p0_hello.html

# Compiling on Microsoft Windows
Install the precompiled toolchain for windows from xpack:
https://github.com/xpack-dev-tools/riscv-none-elf-gcc-xpack/releases/tag/v14.2.0-3

It does NOT work with the precompiled toolchain from here:
https://gnutoolchains.com/risc-v/

```
cd dev/riscv
git clone https://github.com/yhzhang0128/egos-2000.git
cd egos-2000
git checkout hello
```

Patch the makefile:
The patch will enable you to use the path to your own compiler toolchain.
This is usefull in case you have installed a toolchain on windows.

```
#prefix		= C:/SysGCC/risc-v/bin/riscv64-unknown-elf
prefix 		= C:/Users/lapto/Downloads/xpack-riscv-none-elf-gcc-14.2.0-3-win32-x64/xpack-riscv-none-elf-gcc-14.2.0-3/bin/riscv-none-elf

QEMU        = qemu-system-riscv32
RISCV_CC    = $(prefix)-gcc
OBJDUMP     = $(prefix)-objdump
OBJCOPY     = $(prefix)-objcopy
```

An alternative is to adjust the prefix so that it matches the tools inside the 
bin folder of your toolchain and then add the toolchain folder to the PATH variable
in the msys2 64 bit console.

```
PATH=/c/Users/lapto/Downloads/xpack-riscv-none-elf-gcc-14.2.0-3-win32-x64/xpack-riscv-none-elf-gcc-14.2.0-3/bin:$PATH
```

open msys2 64bit console

```
cd /c/Users/lapto/dev/riscv/egos-2000
cd /c/Users/lapto/dev/riscv/egos/src/P0_Hello_World
make clean
make all
```

# P01 Hello World

Official Documentation: https://egos.fun/book/p0_hello.html

The example P01 starts out with purely self-contained source code, then proceeds to include the headers 

```
#include <string.h>  // for strlen() and strcat()
#include <stdlib.h>  // for itoa()
#include <stdarg.h>  // for va_start(), va_end() and va_arg()
```

As a remark, these headers are provided by the toolchain, i.e. by the compiler although they are part of the C standard library.
This means that even though the C standard library is used, this code will compile using the XPack riscv toolchain.

For printing a character, printf() calls terminal_write() and terminal_write() writes bytes to 0x10000000 to print them.
So as an emulator developer, print bytes written to 0x10000000 to stdout to see the output.